...
class HangmanBoard(discord.ui.View):
    ...
    def get_word(self):
        words = ["HELLO", "GOODBYE", "PEACHES", "TOMAHAWK", "PEPPER", "ARRANGE", "DISORDER", "COLLECT", "DOLL",
                 "WITNESS", "LAKE", "WORM", "HESITATE", "FORWARD", "EXTEND", "COMPLAIN"]
    ...
    async def a_cb(self, interaction, button):
        if self.check_guess_and_update(button.label, button) is False and self.hits[0] == 0:
            ...
            await interaction.response.edit_message(content=f"you have {self.hits[0]} left! Current word: {self.found_letters}", view=self)
